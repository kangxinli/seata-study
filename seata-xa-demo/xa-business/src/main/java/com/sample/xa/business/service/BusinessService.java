package com.sample.xa.business.service;
...
import com.sample.xa.business.feign.OrderFeignClient;
import com.sample.xa.business.feign.StorageFeignClient;
...
public class BusinessService {
    ...
    public boolean validData(String userId, String commodityCode) {
        Map<String, Object> accountMap = jdbcTemplate.queryForMap("select * from account_tbl where user_id='" + userId + "'");
        if (Integer.parseInt(accountMap.get("money").toString()) < 0) {
            // 余额被扣减为负：余额不足
            return false;
        }

        Map<String, Object> storageMap = jdbcTemplate.queryForMap(
